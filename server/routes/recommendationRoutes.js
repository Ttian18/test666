...
import Controller from "../feature3/Controller.js";
import { createError, handleError } from "../feature3/errors.js";
import cache from "../feature3/cache.js";
...
router.post("/recommend", upload.single("image"), async (req, res) => {
  try {
    ...
    const recommender = new Controller();
    let result;

    // Check cache logic
    if (cache.hasSameMenu(req.file.buffer)) {
      // Same menu, check if budget is also the same
      if (cache.hasSameBudget(budget)) {
        // Both menu and budget are the same, return cached result
        const cached = cache.getLastRecommendation();
        return res.json({
        ...
        const cachedMenuInfo = cache.getCachedMenuInfo();
...
router.get("/last", (req, res) => {
  const lastRecommendation = cache.getLastRecommendation();
...
router.post("/rebudget", async (req, res) => {
  try {
    ...
    const currentCache = cache.getLastRecommendation();
    cache.setLastRecommendation({
