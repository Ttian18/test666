...
function App() {
  ...
  const handleSocialSubmit = async (e) => {
    ...
    try {
      ...
      const data = await response.json();
      const normalized = {
        originalFilename: data.result?.originalFilename ?? data.originalFilename ?? (data.extractedInfo?.original_filename ?? ""),
        restaurantName: (data.restaurant_name ?? data.result?.restaurantName) ?? "",
        dishName: (data.dish_name ?? data.result?.dishName) ?? null,
        address: (data.address ?? data.result?.address) ?? null,
        description: (data.description ?? data.result?.description ?? data.extractedInfo?.description) ?? "",
        socialMediaHandle: (data.social_media_handle ?? data.result?.socialMediaHandle) ?? null,
        processedAt: data.result?.processedAt ?? new Date().toISOString(),
      };
      setSocialAnalysis(normalized);
    } catch (err) {
  ...
  const handleMenuRecommendSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${BACKEND_URL}/restaurants/recommendations/recommend`, {
        method: "POST",
        body: formData,
      });
  ...
  const handleRebudget = async () => {
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/restaurants/recommendations/rebudget`, {
        method: "POST",
  ...
  const handleFetchLast = async () => {
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/restaurants/recommendations/last`);
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.error || `HTTP ${response.status}`);
      }
  ...
  // Transactions: API helpers using TransactionService
  const fetchVouchers = async () => {
    try {
      setVouchersLoading(true);
      const vouchers = await TransactionService.getVouchers(
        parseInt(txUserId) || 1
      );
  ...
  const handleReceiptUpload = async (e) => {
    ...
    // Validate file
    const validationError = validateFile(receiptFile);
    if (validationError) {
    ...
    try {
      ...
      const result = await TransactionService.uploadReceipt(
        receiptFile,
        parseInt(txUserId) || 1
      );
  ...
  const handleBulkUpload = async (e) => {
    ...
    try {
      ...
      const result = await TransactionService.bulkUploadReceipts(
        bulkFiles,
        parseInt(txUserId) || 1
      );
  ...
  const saveVoucherEdit = async (id) => {
    try {
      ...
      await TransactionService.updateVoucher(
        id,
        payload,
        parseInt(txUserId) || 1
      );
  ...
  const confirmVoucher = async (id) => {
    try {
      ...
      await TransactionService.confirmVoucher(id, {}, parseInt(txUserId) || 1);
  ...
  const deleteVoucher = async (id) => {
    try {
      ...
      await TransactionService.deleteVoucher(id, parseInt(txUserId) || 1);
      ...
      {activeTab === "transactions" && (
        <>
          ...
          <div className="results" style={{ marginTop: 16 }}>
            ...
            <div className="restaurant-card">
              <div className="menu-summary__title">Transaction Summary</div>
                  ...
                  return (
                    <>
                      ...
                      <div className="stat-card">
                        ...
                        <div className="stat-label">Pending Vouchers</div>
                      </div>
                    </>
                  );
                })()}
                          ...
                          <tr key={v.id}>
                            ...
                            <td>
                              {isEditing ? (
                                ...
                                p.category || p.transaction_category || "—"
                              )}
                            </td>
                            <td>
